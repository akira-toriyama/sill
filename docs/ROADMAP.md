...
- 現行 **v1.9.0**（#2 完了・PR #53 merged + tagged 2026-06-19）。**番号が小さいほど先**にやる。
...
3. **`ThemedChip` を新規 — 🚧 着手中（PR #—・→ `v1.10.0`）。** facet のタグ・ステータス pill・キー記号（⌘⇧）を **1部品**で。perch のヒント pill は当初想定に挙がったが、調査で **オーバーレイは CG ループ描画（`OverlayCanvas.PillLayout`／N個を bespoke アニメ）＝ NSView 1チップでは代替不可**と判明し**スコープ外**に確定（必要なら将来 Spec＋描画ヘルパで別途）。
