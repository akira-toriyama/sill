...
1. **ワークスペースまるごとの並べ替え DnD** — 🚧 **着手中: PR #49**（sill **v1.6.0** — ThemedList chunk reorder。ヘッダ＋配下の行を1かたまりで並べ替え／節境界キーボード／grip・節バー。マージで完了＋`v1.6.0` タグ）。⚠ **重要・復帰確定**。facet の「ワークスペース1個を掴んで順番を入れ替える」操作。
