...
複雑な stateful widget（List → ComboBox/Menu）から **Foundation のみ・Sendable・CLT で
`swift test` 可能な純粋核**を抽出し（新規 module `ListCore`）、**controlled/uncontrolled
seam** を契約として明文化し、**a11y を監査して通知欠落と value 属性を補完**する。
すべて**バイト等価リファクタ + additive**（見た目/挙動の変化なし）。
...
- 新規 `ListCoreTests`（CLT で XCTest 可）+ 既存 full-Xcode-only seam テストの repoint/複製。
...
- **DnD/幾何核の移設**（`resolveDropTarget`/`dragCandidates`/`chunkMemberIDs`/`stickyHeader` 等）:
  既に pure かつ test seam 有り（ただし full-Xcode CI のみ）。ListCore へ移せば CLT 化できるが、
...
`#if canImport(CoreGraphics)` で gate＝Motion/Gesture と同テンプレ）。**CLT で `swift test` 可能**
＝ #15 の主目的（核ロジックを maintainer の CLT-only マシンでテストできる）。
...
### テスト
- 新規 **`ListCoreTests`**（A〜D を CLT で XCTest）。
...
- 複雑 stateful なら**純粋核（ListCore 流儀）にロジックを置き CLT テスト**

## 7. 検証方針

- **`swift build` 緑 = ローカル唯一のゲート**（CLT・Xcode 無し）。
- **`ListCoreTests`（新規・CLT で `swift test` 可）** = #15 の主目的の達成証明。
...
2. **CLT test 盲点**: `fittingWidth`（NSString.size/NSFont 呼び）は **Foundation 純粋でない**＝核に入れない
   （full-Xcode のみ）。`_moveHighlight` 等の seam は移設先で orphan させない。
