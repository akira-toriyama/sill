...
""",
    ...
    KitComponent(
        ...
        variants: [
                 ...
                 "preview seam: previewOpen / previewHighlight force deterministic open+highlight (DEBUG comboProbe); on the SwiftUI front as .previewOpen(_:)/.previewHighlight(_:). The frozen popup is still a CHILD WINDOW — absent from a host-window capture; the bench draws the real hosted list in-window instead",
             ],
        family: .text,
...
""",
    ...
    KitComponent(
        ...
        keyAPI: [
                 ...
                 "flashToken: Int — bump to roll a focus/WS-switch blink burst (rolled internally on the view's own clock so the epoch matches); a view born with a non-zero token rolls it at entrance, anchored at attach so a slow cold summon cannot expire it before the first frame (0 = never flashed, no entrance blink)",
...
""",
    ...
    KitComponent(
        ...
        keyAPI: [
                 ...
                 "previewVisible: Bool — test/capture seam: force-show, skip delays+fade; on the SwiftUI front as .themedTooltip(_:palette:placement:previewVisible:). The frozen bubble is still a CHILD WINDOW — absent from a host-window capture; the bench draws ThemedTooltip.previewBubble in-window instead",
...
""",
    ...
    KitComponent(
        ...
        keyAPI: [
                 ...
                 "onActivate(ID) — click/Enter on a row (host's 実処理); onHover(ID?) — hovered row OR section-header id (nil on exit; the veil lights rows only — a host anchors its header hover-preview from this + onRowRects)",
                 ...
                 "DRAG SEAMS (copy-methods, never init labels): .dropTargetValidator((DragContext,DropTarget)->Bool) pre-validates every placement on BOTH input paths (rejected ⇒ no affordance, no keyboard aim, no onDrop) · .dragSourceValidator((ID)->Bool) vetoes BEGINNING a lift without isDisabled's click/hover/selection cost (vetoed ⇒ no ghost, no dim, Space inert; a frozen preview obeys the same gate) · .dragChunk((ID)->[ID]) overrides what a lift carries — [] lifts a header ALONE so it can aim .onto another header (facet's workspace swap) · .dropBand((DropTarget)->[ID]) paints the rows a coarse-granularity drop really lands in as a filled band instead of a 2pt row-level promise",
             ],
        variants: [
                 ...
                 "selectionInk: .wash (selection fill, default) / .solidAccent (opaque primary + onPrimary ink) · showsSelectionAccentBar (the 3pt primary bar on a wash, default true) · roundedSelection picks the SHAPE, independently",
                 ...
                 "showsHoverFill (default false): paint `hover` under ANY enabled hovered .row, selected or not — the click-target-tree reading (facet); a row already drawing a selection/highlight fill keeps it (never painted over). showsPointerAffordances (default false): rows AND headers vend the link pointer — one hover shape for every actionable row (t-1y9q retired the header's open-hand grab) — and a live pointer lift holds the closed hand (.grabActive) across the whole list until the drop; active-app-only per macOS. Standalone hover REPORTS headers too (onHover) — the veil stays row-only; hosted tracking always did",
                 "row statements (post-construction vars + copy-methods, never init labels): ListItem.isEmphasized/.emphasized() — the active row, accent ink + one weight notch, no reflow; ListItem.isDimmed/.dimmed() — parked/faded (facet's hidden windows), the drag-lift veil as a STANDING state, keeping click/hover/selection/drag/AX that isDisabled would take down. A section header draws its ListItem.image as the KIND glyph (template ⇒ the header's title ink, emphasis included; no column reserved when absent). showsTitleTooltips (default false): hover tips carry each row's full title + secondary/sub-line — the read-the-whole-title affordance where horizontal scroll is off",
                 ...
                 "drag affordance: .onto lights the target row (2pt primary ring + faint fill); .between draws a 2pt primary insertion line + leading dot in the gap; the lifted source row dims. A CHUNK lift dims EVERY member row and draws a THICKER full-bleed section insertion bar; the overlay ghost is the members' capped (60%) union with an 'N items' badge. the grid's own pointer DnD is in sill (ThemedGrid entry); rail/real-window drag stay app-side",
...
""",
        ...
        appkitEscape: "ThemedMenu ITSELF (module ThemeKitUI, Sources/ThemeKitUI/ThemedMenu.swift) is an NSObject CONTROLLER — not an NSView — that owns a borderless non-key ThemeKit PopupPanel; the trigger view is SwiftUI-native (ThemedButtonView + the shared PopupAnchorProxy supplying the present(from:) anchor). Only reach for the controller directly (skipping the trigger view) if you need the controller API (ThemedMenu.make(palette:items:) + present(from:)/present(at:in:)/dismiss()/onOpenChange) outside a SwiftUI tree, e.g. a context menu opened from a raw NSEvent."),
    KitComponent(
        ...
        summary: "100% SwiftUI-native grid: responsive LazyVGrid/LazyHGrid layout (ScrollView + GeometryReader), themed cell chrome (rest/hover/selected/keyboard-focused ring+fill+shadow, canonical roles), controlled/uncontrolled selection, 2D keyboard nav (onMoveCommand, a single roving cursor) + double-click/Return activation. ThemedThumbnailGridView supplies the default cell (image fill or a SwiftUI shimmer placeholder + optional bottom-scrim label) over ThemedGridView's generic cell-content seam; themed by passing a ResolvedPalette. Pointer DnD is opt-in via .draggable(mode:) — pure GridCore resolution (onto a cell / row-major insertion slot / both by in-cell fraction, flick-away abort margin) gated by .dropTargetValidator(_:), committed through .onGridDrop(_:); the grid draws the target ring (.onto) or the 2pt insertion line with end caps (.at, the list insertionLine's grid twin) + dims the lifted source + renders an in-bounds SwiftUI drag ghost. Keyboard lift is bound by the kit (t-mej6): Space lifts the cursor cell, arrows aim GEOMETRICALLY through the validated gridDragCandidates, Return/Space commits, Esc cancels. Behaviour opt-ins: .fitsViewport() shrinks cells so every row fits the container with no scrolling (the overlay-grid rule), .activatesOnClick() makes a single click fire onActivate, .wrapsCursor() wraps the arrow cursor at the edges. NO AppKit (#17e AppKit policy) — carousel/hero rail is still out of scope.",
        ...
        keyAPI: [
                 ...
                 "keyboard: onMoveCommand arrow keys move a single roving cursor that REPLACES the selection (macOS grid convention; .wrapsCursor() wraps at edges); Return activates the cursor; on a .draggable grid Space lifts the cursor cell, arrows aim, Return/Space commits, Esc cancels",
                 "GridCellState (isSelected/isHovered/isFocused + isDropTarget/isSwapSource) is handed to a custom cell builder so it can layer its own emphasis atop the kit's chrome",
        ...
        variants: [
                 ...
                 "drag affordance (.draggable grids): the resolved .onto target lights up (2pt primary ring + faint fill, the list's ring); the lifted source cell dims to 40%; the ghost re-renders the REAL cell at the pointer with a primary hairline + shadow; a pointer past the 32pt escape margin resolves nothing (release = abort). Insertion slots (.at) draw the 2pt primary insertion line with 6pt end caps at the slot edge",
                 "carousel/hero rail is OUT OF SCOPE (facet #18 F2 decides its home); keyboard lift: Space lift → arrow aim → Return/Space commit → Esc cancel (t-mej6)",
             ],
        family: .collection,
