...
""",
    ...
    KitComponent(
        ...
        variants: [
                 ...
                 "drag affordance (.draggable grids): the resolved .onto target lights up (2pt primary ring + faint fill, the list's ring); the lifted source cell dims to 40%; the ghost re-renders the REAL cell at the pointer with a primary hairline + shadow; a pointer past the 32pt escape margin resolves nothing (release = abort). Insertion slots (.at) resolve + validate but draw no indicator yet",
                 "carousel/hero rail is OUT OF SCOPE (facet #18 F2 decides its home); the kit binds no keyboard lift yet",
             ],
        family: .collection,
