...
""",
    ...
    KitComponent(
        ...
        variants: [
                 ...
                 "showsHoverFill (default false): paint `hover` under ANY enabled hovered .row, selected or not — the click-target-tree reading (facet); a row already drawing a selection/highlight fill keeps it (never painted over). showsPointerAffordances (default false): rows vend the link pointer, draggable headers the open-hand grab (the grip's cursor twin); active-app-only per macOS. Standalone hover REPORTS headers too (onHover) — the veil stays row-only; hosted tracking always did",
